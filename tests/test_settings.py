...
def test_noise_suppression_defaults():
    s = Settings()
    assert s.mic_noise_suppression_enabled is False
    assert s.mic_noise_suppression_level == "high"


def test_noise_suppression_round_trip(tmp_path: Path):
    s = Settings(mic_noise_suppression_enabled=True, mic_noise_suppression_level="very-high")
    cfg = tmp_path / "config.toml"
    s.save(cfg)
    loaded = Settings.load(cfg)
    assert loaded.mic_noise_suppression_enabled is True
    assert loaded.mic_noise_suppression_level == "very-high"
